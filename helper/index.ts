...
import { Page, BlogPosts, TopicPosts } from "../typescript/pages";
...
export const getTopicListRes = async (): Promise<TopicPosts[]> => {
  const response = (await getEntry({
  ...
  liveEdit &&
    response[0].forEach((entry) => addEditableTags(entry, "topic_full", true));
  return response[0];
};
...
export const getTopicPostRes = async (entryUrl: string): Promise<TopicPosts> => {
  const response = (await getEntryByUrl({
    contentTypeUid: "topic_full",
    entryUrl,
    referenceFieldPath: ["author", "related_post"],
  ...
  liveEdit && addEditableTags(response[0], "topic_full", true);
  return response[0];
};
